...
export const useStaffStore = create<StaffState>((set, get) => ({
        ...
        try {
            ...
            set((state) => ({
                staff: state.staff.map((s) =>
                    s.id === id ? (updatedStaff as unknown as Staff) : s,
                ),
                selectedStaff:
                    state.selectedStaff?.id === id
                        ? (updatedStaff as unknown as Staff)
                        : state.selectedStaff,
            }));
        } catch (error) {
    ...
    updateSchedule: async (id, schedule) => {
        ...
        try {
            ...
            set((state) => ({
                staff: state.staff.map((s) =>
                    s.id === id ? (updatedMember as unknown as Staff) : s,
                ),
                selectedStaff:
                    state.selectedStaff?.id === id
                        ? (updatedMember as unknown as Staff)
                        : state.selectedStaff,
            }));
        } catch (error) {
