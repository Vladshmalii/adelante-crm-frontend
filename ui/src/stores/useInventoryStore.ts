...
interface InventoryState {
    ...
    createStockMovement: (
        data: Omit<StockMovement, 'id' | 'createdAt' | 'createdBy'>,
    ) => Promise<void>;
...
export const useInventoryStore = create<InventoryState>((set) => ({
    ...
    createProduct: async (data) => {
        set({ isLoading: true });
        try {
            const payload = { isActive: true, ...data } as Omit<
                import('@/lib/api/inventory').Product,
            ...
            if (data.productId) {
                ...
                set((state) => ({
                    products: state.products.map((p) =>
                        p.id === data.productId ? updatedProduct : p,
                    ),
                    selectedProduct:
                        state.selectedProduct?.id === data.productId
                            ? updatedProduct
