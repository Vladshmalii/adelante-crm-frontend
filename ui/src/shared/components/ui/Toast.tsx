...
export function Toast({ id, title, message, variant = 'info', onClose }: ToastProps) {
    ...
    return (
        ...
        >
            <div className={clsx('shrink-0 mt-0.5', styles.iconColor)}>{styles.icon}</div>

            <div className="flex-1 min-w-0">
                {title && <p className="text-sm font-semibold mb-1 text-foreground">{title}</p>}
                <p
                    className={clsx(
                        'text-sm',
                        title ? 'text-muted-foreground' : 'text-foreground font-medium',
                    )}
                >
                    {message}
                </p>
