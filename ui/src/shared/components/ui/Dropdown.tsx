...
export function Dropdown({
        ...
        if (buttonRef.current) {
            ...
            if (contentRef.current) {
                ...
                contentHeight = Math.min(
                    searchHeight + optionsCount * itemHeight + groupsHeight,
                    300,
                );
            }
    ...
    const dropdownContent = coords && shouldRender && (
        <div
            ...
            className="fixed z-[9999] flex max-h-[300px] flex-col overflow-hidden rounded-xl border-2 border-border bg-card backdrop-blur-sm"
            style={{
                top: coords.top,
                left: coords.left,
                width: coords.width,
                ...
                <div className="border-b border-border p-2">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Пошук..."
                            className="w-full rounded border border-border bg-background py-1.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                        ...
                        <div key={groupName} className="border-b border-border/50 last:border-0">
                            {grouping && (
                                <button
                                    ...
                                    className={clsx(
                                        'sticky top-0 flex w-full items-center justify-between bg-muted/20 px-3 py-2 text-xs font-bold text-muted-foreground backdrop-blur-sm transition-colors',
                            ...
                            <div
                                className={clsx(
                                    ...
                                    collapsible && !expandedGroups[groupName]
                                        ? 'max-h-0'
                                        : 'max-h-[1000px]',
                                )}
                            >
                                    ...
                                    return (
                                        ...
                                        >
                                            {option.icon && (
                                                <div
                                                    className={clsx(
                                                        'flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-muted/50',
            ...
            {multiple && (
                <div className="border-t border-border bg-muted/10 p-2">
                    <button
                        ...
                        className="w-full py-1.5 text-xs text-muted-foreground transition-colors hover:text-foreground"
                    >
                        Скинути вибір
                    </button>
                </div>
    ...
    return (
        <div ref={dropdownRef} className="relative">
            ...
            <button
                ...
                disabled={disabled}
                className={`flex h-[42px] w-full items-center justify-between gap-2 rounded-xl border bg-background px-4 text-sm transition-all duration-200 ${
                    disabled
                        ? 'cursor-not-allowed opacity-50'
                        : 'cursor-pointer hover:border-primary/50 hover:bg-primary/[0.01]'
                } ${error ? 'border-destructive' : 'border-border/50'} ${isOpen ? 'border-primary ring-2 ring-primary/20' : ''} ${className} `}
            >
                <span
                    className={clsx(
                        'truncate',
                ...
                <ChevronDown
                    size={16}
                    className={`flex-shrink-0 text-muted-foreground/60 transition-transform ${isOpen ? 'rotate-180' : ''}`}
                />
            </button>
