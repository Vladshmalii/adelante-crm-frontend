...
interface EditOperationModalProps {
    ...
    onSave: (data: FinanceOperation) => void | Promise<void>;
}
...
export function EditOperationModal({
        ...
        return (
            <Modal isOpen={isOpen} onClose={onClose} title="Редагувати операцію" size="md">
                <div className="text-sm text-muted-foreground">
                    Оберіть операцію для редагування.
                </div>
            </Modal>
        );
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося оновити операцію',
            );
        }
    };

    const handleChange = <K extends keyof FinanceOperation>(
        field: K,
        value: FinanceOperation[K],
    ) => {
