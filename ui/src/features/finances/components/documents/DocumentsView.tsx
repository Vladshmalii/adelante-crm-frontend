...
export function DocumentsView() {
    ...
    const { documents, isLoading, error, loadDocuments, createDocument } = useFinances({});
    ...
    const handleSaveEdit = () => {
        toast.info(
            'Редагування',
            USE_MOCK_DATA
                ? 'Демо режим: зміни не зберігаються'
                : 'API не підтримує оновлення документів',
        );
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="mb-6 flex items-center justify-between border-b border-border bg-card p-4">
                <h2 className="font-heading text-lg font-semibold">Документи</h2>
                <Button
                    onClick={() => setIsCreateModalOpen(true)}
                    variant="primary"
                    className="flex h-[42px] items-center gap-2 rounded-xl px-6 font-bold"
                >
