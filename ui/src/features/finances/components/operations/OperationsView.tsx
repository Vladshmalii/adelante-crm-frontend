...
export function OperationsView() {
    ...
    const { operations, isLoading, error, loadOperations, createOperation } = useFinances({
        dateFrom,
        dateTo,
    ...
    const handleSaveEdit = () => {
        toast.info(
            ...
            USE_MOCK_DATA
                ...
                : 'API не підтримує оновлення операцій',
        );
    ...
    const filteredOperations = useMemo(() => {
        ...
        if (paymentMethod !== 'all') {
            result = result.filter(
                (op) => (op as any).paymentMethodId?.toString() === paymentMethod,
            );
        }
        return result;
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="mb-6 flex items-center justify-between border-b border-border bg-card p-4">
                <h2 className="font-heading text-lg font-semibold">Операції</h2>
