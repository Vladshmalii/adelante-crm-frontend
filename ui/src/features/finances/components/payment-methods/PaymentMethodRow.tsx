...
export function PaymentMethodRow({ method, onEdit, onToggle, onDelete }: PaymentMethodRowProps) {
    ...
    return (
        <tr className="border-b border-border transition-colors hover:bg-secondary/50">
            <td className="px-4 py-3 text-sm font-medium text-foreground">{method.name}</td>
            <td className="px-4 py-3 text-sm text-muted-foreground">
                {getTypeLabel(method.type || '')}
            </td>
            <td className="px-4 py-3 text-sm text-muted-foreground">{method.cashRegister}</td>
            <td className="px-4 py-3 text-sm text-muted-foreground">{getCommissionText()}</td>
            <td className="px-4 py-3 text-sm text-muted-foreground">
            ...
            <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                    <Button
                        variant="ghost"
                        size="sm"
                        className="h-10 w-10 rounded-xl bg-secondary/50 p-0 transition-all duration-300 hover:bg-primary/10 hover:text-primary"
                    ...
                    <Button
                        ...
                        size="sm"
                        className={clsx(
                            'h-10 w-10 rounded-xl bg-secondary/50 p-0 transition-all duration-300',
                            method.isActive
                    ...
                    <Button
                        variant="ghost"
                        size="sm"
                        className="h-10 w-10 rounded-xl bg-secondary/50 p-0 transition-all duration-300 hover:bg-red-100 hover:text-red-700"
