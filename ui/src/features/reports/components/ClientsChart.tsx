...
export function ClientsChart() {
    ...
    const newClientsChange = Math.round(
        ((lastMonth.newClients - prevMonth.newClients) / prevMonth.newClients) * 100,
    );
    const returningChange = Math.round(
        ((lastMonth.returning - prevMonth.returning) / prevMonth.returning) * 100,
    );
    ...
    return (
        <Card className="p-6">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h3 className="font-heading text-lg font-semibold">Динаміка клієнтів</h3>
            ...
            <div className="mb-6 grid grid-cols-3 gap-4">
                <div className="rounded-xl bg-primary/5 p-4">
                    ...
                    <div
                        className={`mt-1 flex items-center gap-1 text-xs ${newClientsChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                        {newClientsChange >= 0 ? (
                ...
                <div className="rounded-xl bg-green-500/5 p-4">
                    ...
                    <div
                        className={`mt-1 flex items-center gap-1 text-xs ${returningChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
                    >
                        {returningChange >= 0 ? (
                            <TrendingUp className="h-3 w-3" />
                        ) : (
                ...
                <div className="rounded-xl bg-purple-500/5 p-4">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">клієнтів повертаються</p>
                </div>
            </div>
