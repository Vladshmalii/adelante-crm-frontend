        ...
        >
            {categoryToDelete ? (
                <div className="space-y-4 text-sm text-foreground/80 pb-2">
                    ...
                    <div className="my-4 max-h-40 overflow-y-auto border border-border rounded-lg bg-background/50">
                        <table className="w-full text-left text-xs">
                            <thead className="bg-secondary/40 sticky top-0 backdrop-blur-sm z-10 border-b border-border">
                                <tr>
                                    <th className="px-3 py-2.5 font-medium text-muted-foreground uppercase tracking-wider text-[10px]">
                                        Назва
                                    </th>
                                    <th className="px-3 py-2.5 font-medium text-muted-foreground uppercase tracking-wider text-[10px] w-24">
                                        Артикул
                                    </th>
                                    <th className="px-3 py-2.5 font-medium text-muted-foreground uppercase tracking-wider text-[10px] w-20 text-right">
                                        Залишок
                                    </th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border/40">
                                {relatedProductsList.map((p) => (
                                    <tr key={p.id} className="hover:bg-secondary/20">
                                        <td
                                            className="px-3 py-2 font-bold text-foreground truncate max-w-[200px]"
                                            title={p.name}
                                        >
                                            {p.name}
                                        </td>
                                        <td className="px-3 py-2 text-muted-foreground font-medium opacity-80">
                                            {p.sku}
                                        </td>
                    ...
                    <p className="mb-2 pt-2 text-muted-foreground font-medium">
