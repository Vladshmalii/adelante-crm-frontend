...
import type { ServiceWritePayload } from '@/lib/api/services';
...
export function ServicesLayout() {
    ...
    const buildPayload = (data: AddServiceFormData): ServiceWritePayload => ({
        name: data.name,
    ...
    const handleAddService = async (data: AddServiceFormData) => {
        ...
        try {
            await createService(buildPayload(data));
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося створити послугу',
            );
        } finally {
    ...
    const handleEditService = async (id: string | number, data: AddServiceFormData) => {
        ...
        setIsLocalLoading(true);
        try {
            await updateService(id, buildPayload(data));
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося оновити послугу',
            );
        } finally {
            setIsLocalLoading(false);
        }
    ...
    const handleDeleteService = async () => {
        ...
        try {
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося видалити послугу',
            );
        } finally {
            setIsLocalLoading(false);
