...
export function useAppointments(options: UseAppointmentsOptions = {}) {
    ...
    const mapApiToAppointment = (a: ApiAppointment): Appointment => ({
        id: a.id.toString(),
        staffId:
            a.staffId !== undefined && a.staffId !== null ? a.staffId.toString() : 'unassigned',
            ...
            if (USE_MOCK_DATA) {
                ...
                const parsedStaffId =
                    typeof options.staffId === 'string'
                        ? Number.isNaN(parseInt(options.staffId))
                            ? undefined
                            : parseInt(options.staffId)
                        : options.staffId;
    ...
    const createAppointment = async (data: Omit<Appointment, 'id' | 'createdAt' | 'updatedAt'>) => {
        try {
            ...
            if (USE_MOCK_DATA) {
                ...
                // Реальний API
                const staffIdRaw =
                    typeof data.staffId === 'string' ? parseInt(data.staffId) : data.staffId;
                const payload: any = {
