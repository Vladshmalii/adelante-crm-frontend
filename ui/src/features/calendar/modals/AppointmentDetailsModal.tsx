...
interface AppointmentDetailsModalProps {
    ...
    staff: StaffMember[];
    onUpdate: (id: string, updates: Partial<Appointment>) => void | Promise<void>;
...
export function AppointmentDetailsModal({
    ...
    const [isPhoneVisible] = useState(false);
    ...
    const handleStatusChange = async (status: AppointmentStatus) => {
        ...
        try {
            await onUpdate(appointment.id, { status });
        } catch (err) {
            toast.error(
                'Помилка',
    ...
    const handleVisitComplete = async (visitData: { notes: string; photos: string[] }) => {
        try {
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося завершити візит',
            );
        }
    };
                ...
                <div className="p-4 sm:p-5 border-b border-border bg-card flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Button
                            ...
                            size="sm"
                            onClick={onEdit}
                            className="h-9 w-9 p-0 rounded-xl bg-background border border-border/50 hover:text-primary hover:border-primary/30"
                        >
                        ...
                        <Button
                            ...
                            className="h-9 w-9 p-0 rounded-xl bg-background border border-border/50 hover:text-destructive hover:border-destructive/30"
                        >
            ...
            footer={
                <div className="flex justify-end">
                    <Button
                        variant="primary"
                        onClick={onClose}
                        className="px-10 rounded-2xl shadow-lg shadow-primary/20 font-black uppercase tracking-widest text-[11px]"
                    >
                        Зберегти зміни
                    </Button>
                </div>
