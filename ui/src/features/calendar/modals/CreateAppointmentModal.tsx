...
export function CreateAppointmentModal({
    ...
    useEffect(() => {
        ...
        if (clientSearchQuery.length >= 3) {
            const found = CLIENTS_MOCK.filter((c) => {
                const phoneMatch =
                    queryDigits.length >= 4 && c.phone.replace(/\D/g, '').endsWith(queryDigits);
    ...
    const isTimeSlotOccupied = (staffId: string, date: string, start: string, end: string) => {
        ...
        return (appointments || []).some((apt) => {
            ...
            const aStart = apt.startTime
                .split(':')
                .reduce((h: number, m: string) => h * 60 + Number(m), 0);
            const aEnd = apt.endTime
                .split(':')
                .reduce((h: number, m: string) => h * 60 + Number(m), 0);
    ...
    const findNearestAvailableSlot = () => {
        ...
        const selectedServices = MOCK_SERVICES.filter((s) =>
        ...
        for (let i = 0; i < 48; i++) {
            ...
            const nextStartTime = `${Math.floor((nextStartMins / 60) % 24)
                .toString()
                .padStart(2, '0')}:${(nextStartMins % 60).toString().padStart(2, '0')}`;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        toast.success(
            formData.clientId
                ? 'Запис створено для постійного клієнта'
                : 'Запис створено (новий клієнт)',
            'Успіх',
        );
    };
    ...
    const staffOptions = [
        {
            ...
            group: 'Черга',
            icon: (
                <div className="flex h-full w-full items-center justify-center rounded-full bg-muted text-xs font-bold uppercase text-muted-foreground">
                    ?
                </div>
            ),
        },
        ...staff.map((s) => ({
            ...
            icon: (
                <div className="flex h-full w-full items-center justify-center rounded-full bg-primary/10 text-xs font-bold uppercase text-primary">
                    {s.name
                        .split(' ')
                        .map((n) => n[0])
                        .join('')}
    ...
    const getServiceIcon = (category: string) => {
        if (category.includes('зал')) return <Scissors className="h-4 w-4" />;
        if (category.includes('Нігтьовий')) return <Sparkles className="h-4 w-4 text-pink-500" />;
        if (category.includes('Космет')) return <Sparkles className="h-4 w-4 text-green-500" />;
    ...
    const serviceOptions = MOCK_SERVICES.map((s) => ({
        ...
        group: s.category,
        icon: (
            <div className="flex h-full w-full items-center justify-center rounded-lg bg-secondary/50 text-muted-foreground">
                {getServiceIcon(s.category)}
            </div>
        ),
    }));

    const totalDuration = MOCK_SERVICES.filter((s) =>
    ...
    return (
        <Modal
            ...
            position="right"
            header={
                <div className="flex items-center gap-3 border-b border-border bg-card p-4 sm:p-5">
                    <button
                        onClick={onClose}
            ...
            footer={
                <div className="flex w-full items-center justify-between">
                    ...
                    <Button
                        ...
                        variant="primary"
                        disabled={!!timeError}
                        className="rounded-2xl px-10 font-bold shadow-lg shadow-primary/20"
                        onClick={() => {
        ...
        >
            <form id="create-appointment-form" onSubmit={handleSubmit}>
                <div className="grid min-h-[500px] grid-cols-1 gap-6 lg:grid-cols-[240px_1fr_260px]">
                    ...
                    <div className="space-y-4 lg:border-r lg:border-border/30 lg:pr-6">
                        ...
                        <div className="space-y-2 border-t border-border/30 pt-3">
                            ...
                            <Textarea
                                ...
                                rows={3}
                                className="resize-none rounded-xl border-muted-foreground/20 bg-muted/5 text-xs"
                            />
                        </div>
                        ...
                        <div className="border-t border-border/30 pt-3">
                            <div className="grid grid-cols-2 gap-2">
                                <button
                                    type="button"
                                    className="flex flex-col items-center gap-1 rounded-xl border border-border/50 bg-muted/10 p-2.5 text-muted-foreground transition-all hover:bg-muted/30 hover:text-foreground"
                                >
                        ...
                        {/* Type Selection */}
                        <div className="space-y-2 border-t border-border/30 pt-3">
                            <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-foreground opacity-40">
                                Рівень
                            </label>
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {(
                                ...
                                return (
                                    <div
                                        key={status}
                                        className="cursor-default rounded-full border-2 border-border/50 bg-background px-4 py-2 text-[11px] font-black uppercase tracking-wider text-muted-foreground opacity-60"
                                    >
                                        {labels[status]}
                                    </div>
                                );
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium text-foreground opacity-70">
                                    Ціна (грн)
                                </label>
                                <div className="flex h-[46px] items-center rounded-xl border border-border bg-background p-1 transition-all focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                                    <button
                                        type="button"
                                        onClick={() =>
                                            handleChange(
                                                'price',
                                                Math.max(0, (formData.price || 0) - 50),
                                            )
                                        }
                                        className="flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground transition-all hover:bg-muted hover:text-primary active:scale-90"
                                    >
                                    ...
                                    <input
                                        ...
                                        className="flex-1 bg-transparent text-center text-lg font-black text-primary [appearance:textfield] focus:outline-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                                    />
                                    <button
                                        type="button"
                                        onClick={() =>
                                            handleChange('price', (formData.price || 0) + 50)
                                        }
                                        className="flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground transition-all hover:bg-muted hover:text-primary active:scale-90"
                                    >
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex h-[46px] items-center justify-between rounded-xl border border-border bg-muted/30 px-4">
                                    <div className="flex items-center gap-2 text-muted-foreground">
                                        <Clock className="h-4 w-4" />
                                    </div>
                                    <span className="text-lg font-black text-foreground">
                                        {totalDuration || 30}{' '}
                                        <span className="text-xs font-bold opacity-40">хв</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4 lg:border-l lg:border-border/30 lg:pl-6">
                        <div className="relative mb-6">
                            ...
                            <div
                                className={clsx(
                                    'absolute bottom-0 left-3.5 flex h-[42px] items-center transition-colors',
                            ...
                            {clientSearchQuery.length >= 3 && !selectedClient && (
                                <div className="animate-in fade-in slide-in-from-top-4 absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden rounded-2xl border border-border bg-background shadow-2xl duration-300">
                                        ...
                                        {foundClients.length > 0 ? (
                                            foundClients.map((client) => (
                                                <button
                                                    ...
                                                    className="group flex w-full items-center justify-between border-b border-border p-3 transition-all last:border-0 hover:bg-muted"
                                                >
                        ...
                        <Input
                            ...
                            value={
                                formData.clientId
                                    ? CLIENTS_MOCK.find((c) => c.id === formData.clientId)?.email ||
                                      ''
                                    : ''
                            }
                            onChange={() => {}}
                        ...
                        <div className="space-y-3">
                            <div
                                className="flex cursor-pointer select-none items-center gap-3 rounded-xl border border-dashed border-border bg-muted/10 p-3 transition-all hover:border-primary/50"
                                onClick={() =>
                                    handleChange('isForAnotherPerson', !formData.isForAnotherPerson)
                                }
                            >
                                <Checkbox
                                    ...
                                    onChange={(checked) =>
                                        handleChange('isForAnotherPerson', checked)
                                    }
                            ...
                            {formData.isForAnotherPerson && (
                                <div className="animate-in slide-in-from-top-2 space-y-3 rounded-xl border border-border/50 bg-muted/5 p-3">
                                    <Input
                                        label="Ім'я (необов'язково)"
                                        value={formData.otherPersonName || ''}
                                        onChange={(e) =>
                                            handleChange('otherPersonName', e.target.value)
                                        }
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            handleChange('otherPersonPhone', e.target.value)
                                        }
                                        placeholder="+380 XX XXX XXXX"
                                    />
                                </div>
                        ...
                        {selectedClient && (
                            <div className="animate-in slide-in-from-bottom-2 duration-400 space-y-2 rounded-xl border border-primary/10 bg-primary/5 p-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Button
                                        ...
                                        className="h-7 rounded-lg text-[9px] font-black uppercase text-muted-foreground hover:text-destructive"
                                    >
                                        Скинути
                                    </Button>
                                </div>
                                <div className="grid grid-cols-2 gap-2 border-t border-primary/10 pt-2">
                                    <div>
