...
export function AppointmentCard({ appointment, onClick, style, isAdmin }: AppointmentCardProps) {
    ...
    const getStatusIcon = (size: number = 4) => {
        ...
        switch (appointment.status) {
            ...
            default:
                return (
                    <Clock className={`${iconClass} text-muted-foreground/60`} strokeWidth={2.5} />
                );
        }
    };
    ...
    if (isBreak) {
        return (
            <div
                className="pointer-events-none absolute inset-x-0 z-10 flex items-center justify-center rounded-xl border border-dashed border-border/30 bg-muted/5 transition-all"
                style={style}
            >
    ...
    return (
        <div
            className={clsx(
                'group absolute inset-x-1 cursor-pointer rounded-xl border p-0 transition-all duration-500',
                'overflow-hidden shadow-sm hover:-translate-y-0.5 hover:shadow-xl active:scale-[0.98]',
                'z-20 animate-scale-in hover:!z-[100] hover:scale-[1.02] hover:ring-2 hover:ring-primary/20',
                styles.container,
            )}
            style={style}
            onClick={(e) => {
        ...
        >
            ...
            />
            <div className={clsx('absolute inset-y-0 left-0 w-1 shadow-sm', styles.indicator)} />

            <div
                className={clsx(
                    'relative z-10 flex h-full min-w-0 flex-col',
                    isTiny ? 'justify-center p-0.5' : isSmall ? 'p-1.5' : 'p-2',
                )}
            >
                ...
                >
                    <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                        <span
                            className={clsx(
                                'shrink-0 rounded border border-border/5 bg-background/80 px-1 py-0.5 font-black text-foreground antialiased',
                        ...
                        <span
                            className={clsx(
                                'truncate font-heading font-bold leading-[1.1] tracking-tight text-foreground antialiased',
                    ...
                    {!isTiny && (
                        <div
                            className={clsx(
                                'mt-0.5 shrink-0 rounded border border-border/5 bg-background/40',
                                isSmall ? 'p-0' : 'p-0.5',
                            )}
                        >
                ...
                {!isTiny && (appointment.clientName || appointment.otherPersonName) && (
                    <div
                        className={clsx(
                            'mt-auto flex items-center gap-1.5 rounded border border-border/5 bg-background/30',
                            isSmall ? 'px-1 py-0' : 'px-1.5 py-0.5',
                        )}
                    >
                        <User
                            className={clsx(
                                'h-2.5 w-2.5 shrink-0 opacity-40',
                                appointment.isForAnotherPerson ? 'text-amber-500' : 'text-primary',
                            )}
                        />
                            ...
                            >
                                {appointment.isForAnotherPerson
                                    ? appointment.otherPersonName ||
                                      `Від: ${appointment.clientName}`
                                    : appointment.clientName}
                                {appointment.isForAnotherPerson && (
                                    <span className="ml-0.5 text-[7px] font-black uppercase tracking-widest text-amber-500/80">
                                        (Гість)
                                    </span>
                                )}
                            </span>
                            {(appointment.isForAnotherPerson
                                ? appointment.otherPersonPhone || appointment.clientPhone
                                : appointment.clientPhone) && (
                                <span
                                    className={clsx(
                                        'shrink-0 font-medium text-muted-foreground/60',
                ...
                {isLarge && appointment.notes && (
                    <div className="mt-1 flex items-start gap-1 rounded border border-border/5 bg-muted/10 p-1">
                        <FileText className="mt-0.5 h-3 w-3 shrink-0 text-muted-foreground" />
                        <span className="line-clamp-1 text-[8px] font-medium leading-snug text-muted-foreground">
                            {appointment.notes}
                        </span>
                    </div>
                )}
