...
export function ClientsTableRow({
    ...
    return (
        ...
        >
            ...
            <td className="hidden px-4 py-4 text-sm font-black text-foreground lg:table-cell">
                ₴ {client.totalSpent.toLocaleString('uk-UA')}
            </td>
            <td className="hidden px-4 py-4 text-center text-sm font-bold text-foreground lg:table-cell">
                {client.totalVisits}
            </td>
            <td className="hidden px-4 py-4 text-center text-sm font-bold text-primary xl:table-cell">
                <Badge variant="default" className="border-none bg-primary/10 text-primary">
                    {client.discount}%
                </Badge>
            </td>
            <td className="hidden px-4 py-4 text-[11px] font-medium text-muted-foreground lg:table-cell">
                {formatDate(client.lastVisit)}
            </td>
            <td className="hidden px-4 py-4 text-[11px] font-medium text-muted-foreground xl:table-cell">
                {formatDate(client.firstVisit)}
            </td>
            <td className="px-4 py-4 text-right">
                <ClientActionsMenu onView={onClientClick} onEdit={onEdit} onDelete={onDelete} />
            </td>
        </tr>
    );
}
