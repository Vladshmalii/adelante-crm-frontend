...
export function StaffLayout() {
    ...
    const { staff, pagination, isLoading, error, createStaff, updateStaff, deleteStaff } = useStaff(
        {
            search: searchQuery,
            status: activeStatus,
            page: currentPage,
    ...
    // Фільтрація і пагінація виконуються на бекенді — рендеримо результат як є.
    ...
    // Форма пропонує "роль" одним списком (майстер/адміністратор/менеджер),
    // але створення адміністратора вимагає пароль, якого тут немає — тому
    // в backend завжди йде role=master, а обраний варіант зберігається як
    // посада (position), якщо окрему спеціалізацію не вказали.
    const buildPayload = (data: AddStaffFormData): StaffWritePayload => ({
        ...
        position: data.specialization || ROLE_LABELS[data.role] || data.role,
    ...
    const handleSaveStaff = async (data: AddStaffFormData) => {
        ...
        try {
            await createStaff(buildPayload(data));
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося створити співробітника',
            );
        } finally {
    ...
    const handleUpdateStaff = async (id: string | number, data: AddStaffFormData) => {
        ...
        setIsLocalLoading(true);
        try {
            await updateStaff(id, buildPayload(data));
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося оновити співробітника',
            );
        } finally {
            setIsLocalLoading(false);
        }
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            toast.error(
                'Помилка',
                err instanceof Error ? err.message : 'Не вдалося видалити співробітника',
            );
        } finally {
            setIsLocalLoading(false);
        }
    ...
    // Сам StaffScheduleModal зберігає графік через API і показує toast;
    // тут лише закриваємо модалку.
    ...
    const handleImportExcel = async (file: File) => {
        toast.info(
            'Імпорт',
            USE_MOCK_DATA ? 'Демо режим: імпорт не виконується' : `Імпорт ${file.name}`,
        );
    };
    ...
    return (
        <div className="p-4 sm:p-6">
            ...
            <StaffSegments activeStatus={activeStatus} onStatusChange={handleStatusChange} />
